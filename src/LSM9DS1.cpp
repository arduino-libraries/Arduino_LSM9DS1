...
int LSM9DS1Class::begin()
{
  ...
  writeRegister(LSM9DS1_ADDRESS_M, CTRL_REG2_M, 0x0c);
  ...
  if (readRegister(LSM9DS1_ADDRESS_M, WHO_AM_I) != 0x3d) {
    end();

    return 0;
  }

  writeRegister(LSM9DS1_ADDRESS, CTRL_REG1_G, 0x78); // 119 Hz, 2000 dps, 16 Hz BW
  writeRegister(LSM9DS1_ADDRESS, CTRL_REG6_XL, 0x70); // 119 Hz, 4G

  writeRegister(LSM9DS1_ADDRESS_M, CTRL_REG1_M, 0xb4); // Temperature compensation enable, medium performance, 20 Hz
  writeRegister(LSM9DS1_ADDRESS_M, CTRL_REG2_M, 0x00); // 4 Gauss
  writeRegister(LSM9DS1_ADDRESS_M, CTRL_REG3_M, 0x00); // Continuous conversion mode
...
{
  writeRegister(LSM9DS1_ADDRESS_M, CTRL_REG3_M, 0x03);
...
int LSM9DS1Class::readMagneticField(float& x, float& y, float& z)
{
  ...
  if (!readRegisters(LSM9DS1_ADDRESS_M, OUT_X_L_M, (uint8_t*)data, sizeof(data))) {
    x = NAN;
    y = NAN;
    z = NAN;
...
{
  if (readRegister(LSM9DS1_ADDRESS_M, STATUS_REG_M) & 0x08) {
    return 1;
  }
